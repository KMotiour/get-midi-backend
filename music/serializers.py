...
# Olivia Rodrigo - 1 step forward, 3 steps back (in the key of D Major).mid
        ...
        for channel in youTubeChannelId:
            youtubeSearch = requests.get(f"https://www.googleapis.com/youtube/v3/search?part=snippet&channelId={channel}&key=AIzaSyC7uOVx4j2ZVndvV0Ixc4FNDGj-Ga-7cZ8&type=video&q={query}").json()
            print(youtubeSearch)
            if youtubeSearch:
                try:
    ...
    # https://www.youtube.com/channel/UCCt3lxbdR5oVVotKhDMJyiQ -> UCCt3lxbdR5oVVotKhDMJyiQ
        ...
        music = Musics.objects.create(title=title,artist=artist,
...
class MusicRUDSerializers(serializers.ModelSerializer):
    ...
    def get_in_favourite(self, obj):
        bools=False
        musics = TopMusicList.objects.get(id=2)
        for i in musics.music.all():
            if obj.id == i.id:
                bools=True
